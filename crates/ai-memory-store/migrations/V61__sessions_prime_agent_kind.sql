-- Expand sessions.agent_kind CHECK for PrimeIntellect prime-agent (`prime-agent`).
--
-- Keep the complete V51 schema, indexes, and scope-pairing triggers while
-- adding the `prime-agent` wire value.
...
CREATE TABLE sessions_new (
    ...
    agent_kind               TEXT NOT NULL CHECK (agent_kind IN ('claude-code','codex','open-code','cursor','gemini-cli','claude-desktop','openclaw','antigravity-cli','omp','pi','prime-agent','crush','grok','zero','devin','kimi-code','kiro-cli','command-code','hermes','pool','zcode','other')),
    cwd                      TEXT,
