...
impl DeleteKind {
    const fn label(self) -> &'static str {
        match self {
            ...
            Self::PrimeAgentExtension => "prime-agent extension",
...
fn build_plan(args: &UninstallArgs) -> anyhow::Result<Vec<PlannedChange>> {
    ...
    if want(crate::cli::UninstallOnly::Hooks) {
        ...
        let prime = install_hooks::prime_extension_path()?;
        push_generated_delete(&mut plan, prime.clone(), DeleteKind::PrimeAgentExtension);
...
fn generated_file_is_ours(path: &Path, kind: DeleteKind) -> bool {
    ...
    match kind {
        ...
        DeleteKind::PrimeAgentExtension => {
            content
                .contains("Auto-generated by `ai-memory install-hooks --agent prime-agent --apply`")
                && content.contains("const AGENT = \"prime-agent\";")
                && content.contains("pi.registerTool")
        }
        DeleteKind::OpenClawEntrypoint => {
...
fn mcp_servers_path(client: McpClient) -> Option<&'static [&'static str]> {
    match client {
        ...
        // prime-agent's user settings.json keeps unrelated keys (models,
        // extensions, …) beside the top-level `mcpServers` map; only the
        // ai-memory entry is stripped.
