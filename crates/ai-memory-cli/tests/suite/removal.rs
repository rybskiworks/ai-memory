...
fn command_with_home(home: &Path) -> Command {
    ...
    command
        ...
        // The same isolation is required for prime-agent's relocatable
        // agent home.
...
#[test]
fn uninstall_prime_extension_deletes_only_generated_bridge_file() {
    let _guard = cli_test_lock();
    let home = tempfile::tempdir().unwrap();
    let extensions = home.path().join(".prime/agent/extensions");
    std::fs::create_dir_all(&extensions).unwrap();
    let extension = extensions.join("ai-memory-prime-agent.ts");
    let user_content = "// user-owned prime-agent extension\n";
    std::fs::write(&extension, user_content).unwrap();
    ...
    std::fs::write(
        &extension,
        "// Auto-generated by `ai-memory install-hooks --agent prime-agent --apply`.\nconst AGENT = \"prime-agent\";\npi.registerTool({ name: \"memory_status\" });\n",
    )
    .unwrap();
    ...
    assert!(
        !extension.exists(),
        "generated prime-agent extension should be deleted"
    );
}
...
#[test]
fn prime_mcp_install_and_uninstall_round_trip_preserves_settings_keys() {
    let _guard = cli_test_lock();
    let home = tempfile::tempdir().unwrap();
    let settings = home.path().join(".prime/agent/settings.json");
    write_file(
        &settings,
        r#"{"defaultModel":"claude-sonnet-4-20250514","extensions":["./extensions/user-ext.ts"],"mcpServers":{"other":{"type":"http","url":"https://other.example/mcp"}}}"#,
    );

    let install = command_with_home(home.path())
        .args(["install-mcp", "--client", "prime", "--apply"])
        .output()
        .unwrap();
    assert!(
        install.status.success(),
    ...
    assert_eq!(
        installed["mcpServers"]["ai-memory"]["enabledTools"],
        serde_json::json!(["memory_query", "memory_read_page"])
    );
    assert_eq!(
    ...
    assert_eq!(
        installed["defaultModel"], "claude-sonnet-4-20250514",
        "install must preserve unrelated settings keys"
    );
    assert_eq!(
        installed["extensions"],
        serde_json::json!(["./extensions/user-ext.ts"]),
        "install must preserve unrelated settings keys"
    );
    ...
    assert_eq!(
        removed["defaultModel"], "claude-sonnet-4-20250514",
        "uninstall must preserve unrelated settings keys"
    );
}
